...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE agendamentos SET status = 'Negado' WHERE id = :id";
    $stmt = $pdo->prepare($sql);

    if ($stmt->execute([':id' => $id])) {
        echo json_encode(['success' => true, 'message' => 'Agendamento Negado com sucesso.']);
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Erro ao recusar o agendamento.']);
    }
} else {
